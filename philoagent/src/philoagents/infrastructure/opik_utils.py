...
def flush_traces() -> None:
    """Explicitly flushes all pending Opik traces to the server."""
    if settings.COMET_API_KEY:
        try:
            client = opik.Opik()
            logger.info("Attempting to flush pending Opik traces...")
            client.flush()
            logger.info("Opik traces flushed successfully.")
        except Exception as e:
            logger.error(f"Failed to flush Opik traces: {e}")
    else:
        logger.warning("Opik API key not set. Skipping trace flush.")
